...
if [[ -z "$TT_METAL_SLOW_DISPATCH_MODE" ]] ; then
    ...
    grep -i "brisc tripped an assert" tmp.log > /dev/null || { echo "Error: couldn't find expected string in command output:" ; cat tmp.log; exit 1; }
    ...
    grep -i "brisc highest stack usage:" generated/watcher/watcher.log > /dev/null || { echo "Error: couldn't find stack usage in watcher log after dump." ; exit 1; }
