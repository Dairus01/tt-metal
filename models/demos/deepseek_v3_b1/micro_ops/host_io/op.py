...
class HostInterface:
    def __init__(
        ...
        if not loopback_mode:
            if self.multi_core_mode:
                # Create multiple socket pairs: one from each H2D core to D2H core
                self.h2d_to_d2h_socket_pairs = []
                for h2d_core in self.h2d_mesh_core_coords:
                    h2d_to_d2h_connection = ttnn.SocketConnection(
                        h2d_core,
                        self.d2h_mesh_core_coord,
                    )
                    socket_memory_config = ttnn.SocketMemoryConfig(ttnn.BufferType.L1, core_to_core_socket_buffer_size)
                    socket_config = ttnn.SocketConfig([h2d_to_d2h_connection], socket_memory_config)
                    socket_pair = ttnn.create_socket_pair(self.mesh_device, self.mesh_device, socket_config)
                    self.h2d_to_d2h_socket_pairs.append(socket_pair)
            else:
                # Original single-core socket connections
    ...
    def _create_d2h_kernel(self):
        if self.multi_core_mode:
            # Multi-core mode: D2H receiver reads from multiple upstream sockets
            d2h_socket_kernel_ct_args = [
                self.d2h_socket.get_config_buffer_address(),
                ttnn.get_global_semaphore_address(self.termination_semaphore),
                self.d2h_page_size,
                self.num_h2d_cores,  # Number of upstream sockets
            ]
            # Add receiver socket config addresses for all upstream sockets
            for socket_pair in self.h2d_to_d2h_socket_pairs:
                d2h_socket_kernel_ct_args.append(socket_pair[1].get_config_buffer_address())  # Receiver socket
