...
class O_PROJ_GATE_MM_RMSNORM_GAMMA_OverlapConfig:
    ...
    * **ffn_norm** — BFP16, (1, 7168), on core (12, 9).
    ...
    Core (0, 8) is already part of the o_proj grid, so the kv_norm
    gamma is appended after the o_proj shard on that core.  Core (12, 9)
    is a new core that holds three gammas back-to-back.
        ...
        default_factory=lambda: ttnn.CoreRangeSet(
            {
                ...
                ttnn.CoreRange(ttnn.CoreCoord(0, 8), ttnn.CoreCoord(7, 9)),  # 16 cores
            }
        )
    )
    ...
    # kv_norm lives on core (0, 8) which is inside the o_proj grid
    ...
    def kv_norm_byte_offset(self) -> int:
        """Offset after the o_proj shard on core (0, 8)."""
        return self.o_proj_shard_bytes
    ...
    def max_shard_bytes(self) -> int:
        return max(
            ...
            self.o_proj_shard_bytes + self.kv_norm_bytes,
            self.gamma_core_total_bytes,
        )

    @property
    def _kv_norm_core_shard_index(self) -> int:
        """Shard index of kv_norm_core within the o_proj cores."""
        ranges = self.o_proj_core_range_set.ranges()
        idx = 0
        for r in ranges:
            cols = r.end.x - r.start.x + 1
            rows = r.end.y - r.start.y + 1
            if r.start.x <= self.kv_norm_core.x <= r.end.x and r.start.y <= self.kv_norm_core.y <= r.end.y:
                offset_y = self.kv_norm_core.y - r.start.y
                offset_x = self.kv_norm_core.x - r.start.x
                return idx + offset_y * cols + offset_x
            idx += cols * rows
        raise ValueError(f"Core {self.kv_norm_core} not in o_proj core set")
...
class KVB12_PROJ_OverlapConfig:
    ...
    kv_b1_proj is HEIGHT_SHARDED on the 8×8 Qnope grid (same cores that
    run matmul3 in pre-SDPA).  kv_b2_proj is WIDTH_SHARDED on the 64
    remaining cores of the full 10×13 grid (excluding the top-left 8×8
    and bottom-right 2×1).

    Both sub-tensors are BFP8 with identical shard byte sizes, so no
    padding is needed.
    ...
    @property
    def kv_b1_shard_shape(self) -> tuple[int, int]:
        """HEIGHT_SHARDED: (H / num_cores, W)."""
        num_cores = self.kv_b1_core_range_set.num_cores()
        return (self.kv_b1_proj_shape[0] // num_cores, self.kv_b1_proj_shape[1])

    @property
    def kv_b2_shard_shape(self) -> tuple[int, int]:
        """WIDTH_SHARDED: (H, W / num_cores)."""
        num_cores = self.kv_b2_core_range_set.num_cores()
        return (self.kv_b2_proj_shape[0], self.kv_b2_proj_shape[1] // num_cores)

    @property
    def kv_b1_tiles_per_shard(self) -> int:
        sh, sw = self.kv_b1_shard_shape
        return (sh // self.tile_h) * (sw // self.tile_w)

    @property
    def kv_b2_tiles_per_shard(self) -> int:
        sh, sw = self.kv_b2_shard_shape
        return (sh // self.tile_h) * (sw // self.tile_w)

    @property
    def kv_b1_shard_bytes(self) -> int:
        return self.kv_b1_tiles_per_shard * self.bfp8_tile_bytes

    @property
    def kv_b2_shard_bytes(self) -> int:
        return self.kv_b2_tiles_per_shard * self.bfp8_tile_bytes

    @property
    def max_shard_bytes(self) -> int:
        return max(self.kv_b1_shard_bytes, self.kv_b2_shard_bytes)
...
class GATE_UP_PROJ_OverlapConfig:
    ...
    def stack_shards(self, weights: torch.Tensor) -> torch.Tensor:
        """Reorder a (K_gate, K_down) weight matrix into stacked form.

        Core *i* receives the block at
        ``(k_idx = i // n_parallel, n_idx = i % n_parallel)``,
        and all blocks are concatenated vertically.

        Returns:
            Tensor of shape :attr:`stacked_shape`.
        """
        sh, sw = self.shard_shape
        num_cores = self.gate_core_range_set.num_cores()
        shards = []
        for i in range(num_cores):
            k_idx = i // self.n_parallel
            n_idx = i % self.n_parallel
            shards.append(weights[k_idx * sh : (k_idx + 1) * sh, n_idx * sw : (n_idx + 1) * sw])
...
class BlitzDecodeWeights:
    ...
    def get_tt_q_ab_proj_and_kv_a_proj_weights(
        ...
        * kv_a_proj (7168, 576) shards are reordered to match the KV cache
        ...
        assert (
            q_b_proj_weights.shape == cfg.q_b_proj_shape
        ), f"q_b_proj_weights must be {cfg.q_b_proj_shape}, got {tuple(q_b_proj_weights.shape)}"
        assert (
        ...
        shuffled = cfg.shuffle_q_b(q_b_proj_weights)
    ...
    def get_tt_o_proj_and_gate_mm_weights(
        ...
        * **kv_norm** — BFP16 (1×32 tiles) appended after the
          o_proj shard on core (0, 8).
        ...
        Layout::
            ...
            -- dkv gamma on o_proj core (0, 8) --
            kv_norm (1, 512) @ offset 557 056 → 1 024 B

            combined: 121 cores, shard = max_shard_bytes
        ...
        # Splice kv_norm into the o_proj shard at core (0, 8)
        kv_norm_raw = BlitzDecodeWeights._pack_bfloat16_1x32(kv_norm)
        assert len(kv_norm_raw) == cfg.kv_norm_bytes
        kv_norm_shard_idx = cfg._kv_norm_core_shard_index
        kv_norm_offset = kv_norm_shard_idx * max_shard_bytes + cfg.kv_norm_byte_offset
        packed[kv_norm_offset : kv_norm_offset + len(kv_norm_raw)] = kv_norm_raw
        ...
        total_cores = o_num_cores + g_num_cores + 1  # +1 for gamma core
    ...
    def get_tt_kv_b12_proj_weights(
        ...
        """Fuse kv_b1_proj and kv_b2_proj into one WIDTH_SHARDED raw buffer.

        kv_b1_proj is the Qnope head-projection weight (matmul3 in
        pre-SDPA).  It is HEIGHT_SHARDED across the 8×8 Qnope grid so
        that each core holds one head's slice.

        kv_b2_proj is the first post-SDPA matmul weight.  It is
        WIDTH_SHARDED across the 64 remaining cores of the 10×13 grid
        (top-right 5×8 + bottom-left 12×2).

        Both sub-tensors are BFP8 with identical shard byte sizes
        (64 tiles × 1088 B = 69 632 B), so no zero-padding is needed.

        Layout::

            -- kv_b1 region: 64 Qnope cores (8×8) --
            kv_b1_proj (8192, 512) as bfloat8_b, HEIGHT_SHARDED
              shard (128, 512) = 64 tiles × 1088 B = 69 632 B

            -- kv_b2 region: 64 remaining cores (5×8 + 12×2) --
            kv_b2_proj (512, 8192) as bfloat8_b, WIDTH_SHARDED
              shard (512, 128) = 64 tiles × 1088 B = 69 632 B

            combined: 128 cores, shard = 17 408 UINT32 (69 632 B)

        Args:
            kv_b1_proj_weights: Raw kv_b1 tensor, shape (8192, 512).
            kv_b2_proj_weights: Raw kv_b2 tensor, shape (512, 8192).

        Returns:
            A list of two :class:`OverlappedTensor` views
            ``[kv_b1_proj, kv_b2_proj]`` that share the same underlying
            fused device buffer (UINT32, ROW_MAJOR, WIDTH_SHARDED).
        """
        cfg = KVB12_PROJ_OVERLAP_CFG

        # -- Validate shapes --------------------------------------------
        assert (
            kv_b1_proj_weights.shape == cfg.kv_b1_proj_shape
        ), f"kv_b1 must be {cfg.kv_b1_proj_shape}, got {tuple(kv_b1_proj_weights.shape)}"
        assert (
            kv_b2_proj_weights.shape == cfg.kv_b2_proj_shape
        ), f"kv_b2 must be {cfg.kv_b2_proj_shape}, got {tuple(kv_b2_proj_weights.shape)}"

        b1_num_cores = cfg.kv_b1_core_range_set.num_cores()
        b2_num_cores = cfg.kv_b2_core_range_set.num_cores()
        b1_shard_h, b1_shard_w = cfg.kv_b1_shard_shape
        b2_shard_h, b2_shard_w = cfg.kv_b2_shard_shape
        max_shard_bytes = cfg.max_shard_bytes
        assert max_shard_bytes % 4 == 0, "shard bytes must be UINT32-aligned"
        ...
        # kv_b1 shards (HEIGHT_SHARDED, BFP8)
        for i in range(b1_num_cores):
            shard_data = kv_b1_proj_weights[i * b1_shard_h : (i + 1) * b1_shard_h, :].contiguous()
            shard_raw = BlitzDecodeWeights._tilize_and_pack_bfp8(shard_data, cfg.tile_h, cfg.tile_w)
            assert len(shard_raw) == cfg.kv_b1_shard_bytes
            packed.extend(shard_raw)
            packed.extend(b"\x00" * (max_shard_bytes - cfg.kv_b1_shard_bytes))

        # kv_b2 shards (WIDTH_SHARDED, BFP8)
        for i in range(b2_num_cores):
            shard_data = kv_b2_proj_weights[:, i * b2_shard_w : (i + 1) * b2_shard_w].contiguous()
            shard_raw = BlitzDecodeWeights._tilize_and_pack_bfp8(shard_data, cfg.tile_h, cfg.tile_w)
            assert len(shard_raw) == cfg.kv_b2_shard_bytes
            packed.extend(shard_raw)
            packed.extend(b"\x00" * (max_shard_bytes - cfg.kv_b2_shard_bytes))

        # -- Build UINT32 tensor on device ------------------------------
        total_cores = b1_num_cores + b2_num_cores
        uint32_per_shard = max_shard_bytes // 4
        ...
        return [
            OverlappedTensor(
                ...
                shard_shape=cfg.kv_b1_shard_shape,
            ...
            OverlappedTensor(
                ...
                shard_shape=cfg.kv_b2_shard_shape,
    ...
    def get_tt_gate_up_proj_weights(
        ...
        gate_stacked = cfg.stack_shards(gate_proj_weights)
        up_stacked = cfg.stack_shards(up_proj_weights)
