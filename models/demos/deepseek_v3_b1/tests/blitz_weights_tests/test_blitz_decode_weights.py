...
# Tests
# ---------------------------------------------------------------------------
def test_q_ab_proj_kv_a_proj_overlap(device):
    ...
    q_b_raw = torch.randn(cfg.q_b_proj_shape, dtype=torch.bfloat16)
    ...
    q_a_out = _create_output_device_tensor(*q_a.tensor_shape, q_a.core_range_set, device, dtype=q_a.dtype)
    q_a_out = CopyToOutput.op(q_a.fused_tensor, q_a_out, byte_offset=q_a.byte_offset)
    q_a_result = ttnn.to_torch(q_a_out)
    ttnn.deallocate(q_a_out)

    # q_b is tile-reshaped to (6144, 32) per shard inside the fused tensor, but
    # the output uses the original shape (1536, 128) per shard -- same 192 tiles,
    # just re-interpreted as a 48x4 tile grid by ttnn.to_torch.
    q_b_out = _create_output_device_tensor(*q_b.tensor_shape, q_b.core_range_set, device, dtype=q_b.dtype)
    q_b_out = CopyToOutput.op(q_b.fused_tensor, q_b_out, byte_offset=q_b.byte_offset)
    q_b_result = ttnn.to_torch(q_b_out)
    ttnn.deallocate(q_b_out)

    kv_out = _create_output_device_tensor(*kv.tensor_shape, kv.core_range_set, device, dtype=kv.dtype)
    ...
    q_a_ref = _get_roundtrip_reference(cfg.shuffle_q_a(q_a_raw), q_a.core_range_set, device, dtype=q_a.dtype)
    assert torch.equal(q_a_result, q_a_ref), "q_a_proj overlap has mismatch"
    logger.info("q_a_proj overlap comparison passed")

    q_b_ref = _get_roundtrip_reference(cfg.shuffle_q_b(q_b_raw), q_b.core_range_set, device, dtype=q_b.dtype)
    assert torch.equal(q_b_result, q_b_ref), "q_b_proj overlap has mismatch"
    logger.info("q_b_proj overlap comparison passed")

    kv_ref = _get_roundtrip_reference(cfg.shuffle_kv_a(kv_raw), kv.core_range_set, device, dtype=kv.dtype)
    assert torch.equal(kv_result, kv_ref), "kv_a_proj overlap has mismatch"
    logger.info("kv_a_proj overlap comparison passed")
...
def test_kv_b12_proj_overlap(device):
    ...
    kv_b1_proj is HEIGHT_SHARDED on the 8×8 Qnope grid.
    kv_b2_proj is WIDTH_SHARDED on the remaining 64 cores.
    """
    ...
    b1_out = _create_output_device_tensor(
        *kv_b1.tensor_shape,
    ...
    b1_out = CopyToOutput.op(kv_b1.fused_tensor, b1_out, byte_offset=kv_b1.byte_offset)
    b1_result = ttnn.to_torch(b1_out)
    ttnn.deallocate(b1_out)

    # -- Extract kv_b2 (WIDTH_SHARDED, BFP8) while fused buffer is alive -----
    b2_out = _create_output_device_tensor(
        *kv_b2.tensor_shape,
    ...
    b2_out = CopyToOutput.op(kv_b2.fused_tensor, b2_out, byte_offset=kv_b2.byte_offset)
    b2_result = ttnn.to_torch(b2_out)
    ttnn.deallocate(b2_out)

    ttnn.deallocate(kv_b1.fused_tensor)

    # -- Build references (bfp8 round-trip) after fused buffer is freed ------
    b1_ref = _get_roundtrip_reference(
        kv_b1_raw,
        kv_b1.core_range_set,
        device,
        dtype=kv_b1.dtype,
    ...
    assert torch.equal(b1_result, b1_ref), "kv_b1_proj overlap has mismatch"
    logger.info("kv_b1_proj overlap comparison passed")

    b2_ref = _get_roundtrip_reference(
        kv_b2_raw,
        kv_b2.core_range_set,
        device,
        dtype=kv_b2.dtype,
    ...
    assert torch.equal(b2_result, b2_ref), "kv_b2_proj overlap has mismatch"
