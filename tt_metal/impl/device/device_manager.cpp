...
const std::unordered_set<CoreCoord>& DeviceManager::get_virtual_dispatch_cores(ChipId dev_id) const {
    ...
    TT_ASSERT(
        it != initializers_.end(), "Dispatch firmware not initialized yet. Call initialize_dispatch_firmware() first.");
    auto* dispatch_kernel_initializer = dynamic_cast<DispatchKernelInitializer*>(it->second.get());
    TT_ASSERT(
